-- 웹툴 모음 데이터베이스 스키마 (안전 버전)
-- 이 버전은 DROP 명령 없이 기존 객체를 확인 후 생성합니다.
...
BEGIN
  ...
  -- 사용 로그 정책
  DROP POLICY IF EXISTS "Users can view own usage logs" ON public.tool_usage_logs;
  DROP POLICY IF EXISTS "Admins can view all usage logs" ON public.tool_usage_logs;
  
  -- 저장 데이터 정책
  DROP POLICY IF EXISTS "Users can manage own saved data" ON public.user_saved_data;
...
-- 트리거: updated_at 자동 업데이트 (profiles)
DO $$ 
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_trigger 
...
-- 트리거: updated_at 자동 업데이트 (user_saved_data)
DO $$ 
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_user_saved_data_updated_at' 
    AND tgrelid = 'public.user_saved_data'::regclass
  ) THEN
    DROP TRIGGER update_user_saved_data_updated_at ON public.user_saved_data;
  END IF;
END $$;
